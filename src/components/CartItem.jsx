...
const CartItem = ({item}) => {
  ...
  return (
    <div className='flex flex-col items-center sm:flex-row justify-between p-4 sm:p-6 mb-4 bg-gray-900 rounded-xl shadow-2xl border-gray-800 
    transition duration-300 hover:border-orange-600/50'>
      ...
      <div className='flex items-center border border-gray-700 rounded-full overflow:hidden shadow-lg'>
        ...
        <button onClick={decreaseQ} className='p-2 text-gray-400 bg-gray-800 hover:bg-gray-700 transition duration-150 w-8 h-8 items-center rounded-full justify-center'>
          -
        </button>
        <span className='px-3 text-base font-bold text-white bg-gray-800'>{item.quantity}</span>

        <button onClick={increaseQ} className='p-2 text-gray-400 bg-gray-800 hover:bg-gray-700 transition duration-150 w-8 h-8 items-center rounded-full justify-center'>
          +
        </button>
      ...
      <button onClick={()=>removeFromCart(item.id,true)} className='p-3 bg-red-800/20 text-red-400 rounded-full hover:bg-red-800/40 
